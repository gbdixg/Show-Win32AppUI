function Add-IntuneWin32AppAssignmentGroupEx {
    <#
    .SYNOPSIS
    ...
    Process {
        ...
        try{
            $Win32App = Invoke-RestMethod -Headers $Global:AuthenticationHeader -Uri $URI -Method Get -ErrorAction Stop
        }catch{
            $Win32App = $null
        }   
            ...
            #$DuplicateAssignment = Test-IntuneWin32AppAssignment -ID $Win32AppID -Target "Group"
                ...
                try {
                    ...
                    try {
                        ...
                        $Win32AppAssignmentResponse = Invoke-RestMethod -Uri $GraphURI -Headers $Global:AuthenticationHeader -Method POST -Body ($Win32AppAssignmentBody | ConvertTo-Json) -ContentType "application/json" -ErrorAction Stop -Verbose
                        
                    }catch [System.Exception] {
                        Write-Warning -Message "An error occurred while creating a Win32 app assignment. Error message: $($_.Exception.Message)"
                        $Win32AppAssignmentResponse = $null
                    }
